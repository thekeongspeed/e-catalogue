...
<body>
    ...
    <div class="container py-4" style="max-width: 600px;">
        <h4 class="mb-4 fw-bold">Kelola Data Konfigurasi</h4>
        ...
        <div class="section-card mb-4">
            <h6 class="fw-bold text-primary mb-3"><i class="fas fa-plus-circle me-2"></i>Tambah Data Konfigurasi</h6>
            <form action="/configurations/store" method="POST">
                @csrf
                <div class="input-group">
                    <input type="text" name="name" class="form-control" placeholder="Nama Konfigurasi..." required>
        ...
        <div class="section-card">
            <h6 class="fw-bold mb-3"><i class="fas fa-list me-2"></i>Daftar Konfigurasi</h6>
            @forelse($configurations as $div)
                <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                    <span>{{ $div->name }}</span>
                    <form action="/configurations/{{ $div->id }}" method="POST" onsubmit="return confirm('Hapus konfigurasi ini?')">
                        @csrf
                        @method('DELETE')
            ...
            @empty
                <p class="text-muted fst-italic">Belum ada data konfigurasi.</p>
            @endforelse
        </div>
    </div>
</body>
